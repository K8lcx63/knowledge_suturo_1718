...
bool find_grasp_pose(knowledge_msgs::GraspIndividual::Request &req, knowledge_msgs::GraspIndividual::Response &res)
{
  ...
  for(PrologQueryProxy::iterator it=bdgs.begin();
      it != bdgs.end(); it++)
    {
      ...
      if(req.object_label == "JaMilch"){res.force = 0.0;}
      if(req.object_label == "TomatoSauceOroDiParma"){res.force = 0.0;}
      if(req.object_label == "SiggBottle"){res.force = 0.0;}
      if(req.object_label == "EdekaRedBowl"){res.force = 0.0;}
      if(req.object_label == "CupEcoOrange"){res.force = 0.0;}
      if(req.object_label == "KoellnMuesliKnusperHonigNuss"){res.force = 0.0;}
      if(req.object_label == "PringlesPaprika"){res.force = 0.0;}
      if(req.object_label == "PringlesSalt"){res.force = 0.0;}
      if(req.object_label == "HelaCurryKetchup"){res.force = 0.0;}
      if(req.object_label == "KeloggsToppasMini"){res.force = 0.0;}
